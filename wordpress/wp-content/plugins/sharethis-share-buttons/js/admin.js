	...
	return {
		...
		loadPreview: function( type, button ) {
			...
			var bAlignment = $( '.button-alignment .alignment-button[data-selected="true"]' ).attr( 'data-alignment' ),
				...
				nativeCounts = $( '#' + button + ' span.native-counts' ).siblings( 'div.switch' ).find( 'input' ).is( ':checked' ),
			...
			if ( 'inline' === button ) {
				config = { alignment: bAlignment,
					...
					use_native_counts: nativeCounts,
					size: size,
					spacing: spacing,
					language: language,
				};
		...
		setConfigFields: function( button, config, type ) {
			...
			$( '#' + button + ' span.native-counts' ).siblings( 'div.switch' ).find( 'input' ).prop( 'checked', ( undefined !== config['use_native_counts'] && 'true' === config['use_native_counts'].toString() ) );
